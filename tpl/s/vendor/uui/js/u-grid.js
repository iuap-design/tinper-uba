...
//								span.removeAttribute("style")
				...
				if(sumCol) {
					var sumSpan = $('#' + this.options.id + '_content_'+idStr+'sum_row').find('td').eq(i).find('span:last-child')[0];
					if(sumSpan) {
						if(typeof sumRenderType == 'function') {
							var sumV = sumSpan.innerHTML;
							var obj = {};
							obj.value = sumV;
							obj.element = sumSpan;
							obj.gridObj = oThis;
		...
		$("div",this.$element).each(function() {
			...
			if(eType == 'string' || eType == 'integer' || eType == 'checkbox' || eType == 'combo' || eType == 'radio' || eType == 'float' || eType == 'currency' || eType == 'datetime'|| eType == 'date' || eType == 'time'){
				if(eType == 'string'){
					...
					comp = new u.OriginText({
						el:compDiv.find("input")[0],
						options:eOptions,
						model: viewModel
					});
