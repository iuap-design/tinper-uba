...
u.Text = u.BaseComponent.extend({
    ...
    init: function () {
        ...
        this._input = this.element.querySelector('.' + this._CssClasses.INPUT);
...
u.Radio = u.BaseComponent.extend({
    ...
    init: function () {
        this._btnElement = this.element.querySelector('.' + this._CssClasses.RADIO_BTN);
...
u.DateTimePicker.fn.init = function(){
    ...
    this._element.style.display = "table"; // 存在右侧图标，因此修改display
...
u.Combo = u.BaseComponent.extend({
    init: function () {
        ...
        this._input = this.element.querySelector(".u-input");
        u.on(this._input, 'focus', function (e) {
            self.show(e);
            u.stopEvent(e);
        })
...
if (u.compMgr)
	...
	u.pagination.prototype.render = function() {
		...
		if (options.jumppage || options.pageSize) {
			var jumppagehtml = '<input class="page_j" style="margin-right: 6px;width: 32px;border: 1px solid #ddd; padding-left: 2px; border-radius: 3px;" value=' + options.currentPage + '>页&nbsp;&nbsp;'
			var sizehtml = '显示<input  class="page_z"  style="margin:0px 6px;width: 32px;border: 1px solid #ddd;  padding-left: 2px; border-radius: 3px;" value=' + options.pageSize + '>条&nbsp;&nbsp;'
		...
		u.on(this.$ul.querySelector(".jump_page"),"click", function() {
			var jp, pz;
			jp = this.$ul.querySelector(".page_j").value || options.currentPage;
			pz = this.$ul.querySelector(".page_z").value || options.pageSize;
...
u.TextFieldAdapter = u.Class.create({
    /**
     *
     * @param comp
     * @param options ：
...
u.CheckboxAdapter = u.BaseAdapter.extend({
    initialize: function (options) {
        ...
        this.eleValue = this.comp._inputElement.value;
        ...
        this.comp.on('change', function(event){
            ...
            if (self.comp._inputElement.checked){
                valueArr.push(self.eleValue)
            }else{
                var index = valueArr.indexOf(self.eleValue);
    ...
    modelValueChange: function (val) {
        if (this.slice) return;
        if (this.comp._inputElement.checked != (val + ',').indexOf(this.eleValue) > -1){
            this.slice = true
            this.comp._inputElement.click();
            this.slice = false
        }
        //if (this.slice) return;
        //val = val || "";
...
u.PaginationAdapter = u.BaseAdapter.extend({
        initialize: function (comp, options) {
            ...
            this.comp.on('pageChange', function (event, size, pageIndex) {
                if (typeof self.sizeChange == 'function') {
                    self.sizeChange(size, pageIndex);
                } else {
                    u.showMessageDialog("没有注册sizeChange事件")
                }
            });
