//define([  'text!pages/school_course/course.html',  'print', 'share', 'qrcode'], function(  template) {    //xzn
$.get("./src/pages/school_course/course.html",function(template){
	var ctrlBathPath = '/school_course';     //xzn
	var app, viewModel, datas;
	var viewModel = {
		md: document.querySelector('#demo-mdlayout'),
		editoradd: '',
		searchText: ko.observable(''),
		searchFileds: ['course_node','course_name'],    //xzn  搜索字段修改
		...
		events: {
			...
			delRow: function() {
		        ...
		        u.confirmDialog({
		            ...
		            onOk: function () {
		                $.ajax({
		                	url:ctx+'/school_course/del',      //xzn
		                	type:'post',
		                	data:{data:JSON.stringify(viewModel.mainDataTable.getSimpleData({type:'select',fields:['course_id','course_node']}))},    //xzn
		                	success: function(){
		                		u.showMessage('删除成功！');
