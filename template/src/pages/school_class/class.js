...
$.get("./src/pages/school_class/class.html",function(template){
	...
	var viewModel = {
		...
		searchFileds: ['class_code','class_name'],    //xzn  搜索字段修改
		...
		events: {
			...
			delRow: function() {
		        ...
		        u.confirmDialog({
		            ...
		            onOk: function () {
		                $.ajax({
		                	url:ctx+'/school_class/del',      //xzn
		                	type:'post',
		                	data:{data:JSON.stringify(viewModel.mainDataTable.getSimpleData({type:'select',fields:['class_id']}))},    //xzn
		                	success: function(){
		                		u.showMessage('删除成功！');
