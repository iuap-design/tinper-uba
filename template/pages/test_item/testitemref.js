define([ 'text!pages/test_item/testitemref.html',  'print', 'share', 'qrcode'], function( template) {    //xzn
	...
	var ctrlBathPath = ctx+'/test_item';     //xzn
	var app, viewModel, datas;
	var viewModel = {
		md: document.querySelector('#demo-mdlayout'),
		editoradd: '',
		searchText: ko.observable(''),
		searchFileds: ['testitemcode','testitemname'],    //xzn  搜索字段修改
		
		mainDataTableRef: new u.DataTable({
			meta: {
				'testitemid':{
					type:'string'
				},
		...
		events: {
			...
			queryMain: function(){
		        ...
		        queryData["pageIndex"] = viewModel.mainDataTableRef.pageIndex();
		        queryData["pageSize"] = viewModel.mainDataTableRef.pageSize();
				$.ajax({
					type : 'GET',
					url : ctrlBathPath+'/testitem',      //xzn
					data : queryData,
					dataType : 'json',
					success : function(result) {
						var data = result.data;
						if(data!=null){
							viewModel.mainDataTableRef.setSimpleData(data.content);
							viewModel.mainDataTableRef.totalPages(data.totalPages);
						} else {
...
//				return true;
//			},
			rowSelectFun: function(row){
				var index = viewModel.mainDataTableRef.getRowIndex(row);
				viewModel.mainDataTableRef.setRowSelect(index);
			},
			...
			addOrEditRow: function() {
				var self = this;
				var _meta = this.mainDataTableRef.meta;
					...
					success:function(res){
						if (res.flag == 'success'){
							if (self.editoradd === 'add'){
								self.mainDataTableRef.addSimpleData(res.data);
							}else{
								var curRow = viewModel.mainDataTableRef.getCurrentRow();
			...
			beforeEdit: function(id) {
				viewModel.mainDataTableRef.setRowSelect(id);
				viewModel.infodata.setSimpleData(viewModel.mainDataTableRef.getSimpleData({
					type: 'select'
				}));
			...
			delRow: function() {
				var selectArray = viewModel.mainDataTableRef.selectedIndices();
		        ...
		        u.confirmDialog({
		            ...
		            onOk: function () {
		                $.ajax({
		                	url:ctx+'/test_item/del',      //xzn
		                	type:'post',
		                	data:{data:JSON.stringify(viewModel.mainDataTableRef.getSimpleData({type:'select',fields:['testitemcode','testitemname']}))},    //xzn
		                	success: function(){
		                		u.showMessage('删除成功！');
			...
			pageChangeFunc: function(index){
				viewModel.mainDataTableRef.pageIndex(index);
				viewModel.events.queryMain();		
			},
			sizeChangeFunc: function(size){
				viewModel.mainDataTableRef.pageSize(size);
				viewModel.events.queryMain();
			}
		}
	};
	...
	var init = function(refer) {
		...
		refer.registerSubmitFunc(function(){
			return viewModel.mainDataTableRef.getSelectedRows()[0]
		})
		var app = u.createApp({
			el: '#table-list-ref',
			model: viewModel
